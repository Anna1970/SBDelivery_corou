...
fun CartFeature.State.selfReduce(msg: CartFeature.Msg): Pair<CartFeature.State, Set<Eff>> =
    when (msg) {
        ...
        is CartFeature.Msg.HideConfirm -> copy(confirmDialog = ConfirmDialogState.Hide) to emptySet()//todo()
        is CartFeature.Msg.RemoveFromCart -> copy(confirmDialog = ConfirmDialogState.Hide) to setOf(CartFeature.Eff.RemoveItem(msg.dishId)).toEffs() //todo()
        is CartFeature.Msg.SendOrder -> {
            this to setOf(CartFeature.Eff.SendOrder(msg.order)).toEffs()
        } //todo()
        ...
        is CartFeature.Msg.ShowConfirm -> copy(confirmDialog = ConfirmDialogState.Show(msg.id, msg.title)) to emptySet()//todo()
    }
